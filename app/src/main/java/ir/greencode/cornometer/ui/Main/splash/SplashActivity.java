...
public class SplashActivity extends BaseActivity {
    ...
    private void decideForSomething() {
        ...
        if(!mySharedPreferencesl.getDataBoolean(Constant.PREF_FIRST)){
            viewModel.insertCat(new MyTimer("Read Aloud(RA)","40 sec wait,40 sec for read",2,40000,40000,R.drawable.reading));
            viewModel.insertCat(new MyTimer("Describe Image(DI)","25 sec wait,40 sec for describe",2,25000,40000,R.drawable.describe));
            viewModel.insertCat(new MyTimer("Retell lecture(RL)","40 sec for retell",1,0,40000,R.drawable.lecture));
            viewModel.insertCat(new MyTimer("Summarize Written Text(SWT)","10 Min for write",1,0,600000,R.drawable.swt));
            viewModel.insertCat(new MyTimer("Essay","20 Min for write",1,0,1200000,R.drawable.essay));
            viewModel.insertCat(new MyTimer("Summarize spoken text","10 Min for hearing and write",1,0,600000,R.drawable.lecture));
